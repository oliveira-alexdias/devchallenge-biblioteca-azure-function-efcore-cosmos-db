...
{
    public class AtualizarObraModel : BaseModel
    {
        public AtualizarObraModel()
        {
        }

        public AtualizarObraModel(string body)
        {
            var model = JsonConvert.DeserializeObject<AtualizarObraModel>(body) ?? new AtualizarObraModel();
        ...
        {
            var validator = new InlineValidator<AtualizarObraModel>();
            ...
            validator.RuleFor(i => i.Editora)
                ...
                .MaximumLength(100)
                .WithMessage("A editora da obra não deve possuir mais que 100 caracteres");

            validator.RuleFor(i => i.Autores)
                .Must(i => i.Count > 0)
                .WithMessage("O título deve possuir pelo menos um autor");
